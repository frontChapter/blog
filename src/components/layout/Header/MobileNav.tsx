'use client'
import clsx from 'clsx'
import { headerNavbar } from 'constants/layout'
import Link from 'next/link'
import React, { Dispatch, SetStateAction } from 'react'

const MobileNavItem = ({ item }) => {
  const [isShowChildren, setIsShowChildren] = React.useState(false)
  const hasChild = !!item?.children?.length
  const classes = isShowChildren
    ? 'opacity-100 visible'
    : 'invisible h-0 opacity-0'

  return (
    <div
      className="flex flex-col font-light border-b border-gray-300 cursor-pointer last-of-type:border-0"
      {...(hasChild
        ? { onClick: () => setIsShowChildren((prev) => !prev) }
        : {})}
    >
      <span
        className={clsx(
          'flex items-center justify-between gap-2 text-base',
          !hasChild && 'py-3'
        )}
      >
        {item.label}
        {hasChild && (
          <div className="flex items-center justify-center flex-none w-10 py-3 h-fit">
            <svg
              className={clsx(
                'w-5 h-5 transition-all duration-300 ease-in-out transform fill-current',
                isShowChildren ? 'rotate-180' : 'rotate-0'
              )}
              xmlns="http://www.w3.org/2000/svg"
              width={20}
              height={20}
              viewBox="0 0 24 24"
            >
              <path d="M7.41,8.58L12,13.17L16.59,8.58L18,10L12,16L6,10L7.41,8.58Z" />
            </svg>
          </div>
        )}
      </span>
      {hasChild && (
        <ul
          className={clsx(
            'flex flex-col transition-opacity duration-500',
            classes
          )}
        >
          {item?.children?.map((child) => (
            <li
              key={child.key}
              className="pr-3 border-b border-gray-300 last-of-type:border-0 first-of-type:border-t hover:bg-gray-300"
            >
              <Link
                className="block py-3 text-base transition link link-hover hover:text-primary hover:duration-300"
                href={child.href}
              >
                {child.label}
              </Link>
            </li>
          ))}
        </ul>
      )}
    </div>
  )
}

const MobileNav = ({
  state,
}: {
  state: [isOpen: boolean, setIsOpen: Dispatch<SetStateAction<boolean>>]
}) => {
  const [isOpen, setIsOpen] = state
  const handleClose = () => setIsOpen(false)

  return (
    <nav>
      <div className="block xl:hidden">
        <div
          className={clsx(
            'overflow-y-auto z-40 flex pt-5 top-0 flex-col h-screen w-full max-w-[300px] fixed bg-gray-200 duration-500 ease-in gap-2 md:gap-0 shadow-xl',
            isOpen ? 'right-0' : '-right-full'
          )}
        >
          <div className="relative flex flex-col gap-5 px-5 pb-6 mt-4 text-lg font-normal leading-6">
            <div className="flex items-center justify-between mb-6">
              <h4 className="text-base">
                <span className="font-bold">فرانت‌</span>
                چپتر
              </h4>
              <svg
                className="cursor-pointer"
                onClick={handleClose}
                width={10}
                height={10}
                viewBox="0 0 10 10"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M1 9L9 1M1 1L9 9"
                  stroke="#6B7280"
                  strokeWidth="1.5"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                />
              </svg>
            </div>
            <div className="flex items-center gap-4 px-2 py-2 border rounded-md md:hidden">
              <input
                type="text"
                className="w-full text-base bg-gray-200 outline-none"
                placeholder="جست‌وجو"
              />
              <svg
                width={16}
                height={16}
                viewBox="0 0 16 16"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M6.90906 2C5.93814 2 4.98903 2.28791 4.18174 2.82733C3.37444 3.36674 2.74524 4.13343 2.37368 5.03045C2.00213 5.92746 1.90491 6.91451 2.09433 7.86677C2.28375 8.81904 2.75129 9.69375 3.43783 10.3803C4.12438 11.0668 4.99909 11.5344 5.95135 11.7238C6.90362 11.9132 7.89067 11.816 8.78768 11.4444C9.6847 11.0729 10.4514 10.4437 10.9908 9.63639C11.5302 8.8291 11.8181 7.87998 11.8181 6.90906C11.818 5.60712 11.3008 4.35853 10.3802 3.43792C9.45959 2.51731 8.211 2.00008 6.90906 2Z"
                  stroke="#52525B"
                  strokeWidth="1.5"
                  strokeMiterlimit={10}
                />
                <path
                  d="M10.5715 10.5716L14 14"
                  stroke="#52525B"
                  strokeWidth="1.5"
                  strokeMiterlimit={10}
                  strokeLinecap="round"
                />
              </svg>
            </div>
            <div className="flex flex-col">
              {headerNavbar.map((item) => (
                <React.Fragment key={item.key}>
                  <MobileNavItem item={item} />
                </React.Fragment>
              ))}
            </div>
          </div>
        </div>
      </div>
      <div
        onClick={handleClose}
        className={clsx(
          'xl:hidden fixed top-0 z-30 transition-all duration-500 ease-in-out bg-[#1B2631] opacity-80 h-full w-full',
          isOpen ? 'right-0' : '-right-full'
        )}
      />
    </nav>
  )
}

export default MobileNav
